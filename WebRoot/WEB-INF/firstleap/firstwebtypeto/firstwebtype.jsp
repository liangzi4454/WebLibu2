			...
			var choose = confirm('是否要删除所选信息');
				if(choose == true) {
					$.ajax({
					   type: "post",
					   dataType: "json",
					   url: "${pageContext.request.contextPath}/webtype/webtype_delete.action",
					   data: {id:this.name},
					   success: function(msg){	
					   		//	如果删除成功，重定向刷新list
					   		if(msg.msgname == "sccg") 
					   		{
					   			window.location = "${pageContext.request.contextPath}/webtype/webtype_listi.action?page=${page}";
					   		}			   		
					 }});
				}
		}); /*****结束****/
													                      		...
													                      		<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="chart_list">
													                          		<tr align="right" background="images/001_57.jpg">
													                            		<td height="27" colspan="10" style="background-repeat:repeat-x;">
													                            			<a href="<%=basePath%>/webtype/webtype_createi.action" target="main" class="text4">增加</a>
													                            		</td>
													                          		</tr>
													                          		...
													                          		<s:iterator id="firstwebtype" value="pagination.list" status="userList">
													                          			<tr align="center" valign="middle">
																                            <td height="27">${userList.count}</td>
																                            <td height="27">${firstwebtype.name}</td>
																                            <td height="27">
																	                          <!-- 	<a href="<%=basePath%>/webtype/webtype_edit.action?id=${firstwebtype.id}" style="cursor:pointer" class="text4">修改</a>&nbsp;&nbsp; -->   
																	                            <a class="deleteButton" name="${firstwebtype.id}" style="cursor:pointer" class="text4"><s:text name="删除" /></a>
																                            </td>
														                           		</tr>
													                          		</s:iterator>
													                          			<tr>
