			...
			var choose = confirm('是否要删除所选信息');
				if(choose == true) {
					$.ajax({
					   type: "post",
					   dataType: "json",
					   url: "${pageContext.request.contextPath}/webart/webart_delete.action",
					   data: {id:this.name},
					   success: function(msg){	
					   		//	如果删除成功，重定向刷新list
					   		if(msg.msgname == "sccg") 
					   		{
					   			window.location = "${pageContext.request.contextPath}/webart/webart_list.action?page=${page}";
					   		}			   		
					 }});
				}
		}); /*****结束****/
													                      		...
													                      		<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="chart_list">
													                          		<tr align="right" background="images/001_57.jpg">
													                            		<td height="27" colspan="10" style="background-repeat:repeat-x;">
													                            			<a href="<%=basePath%>/webart/webart_create.action" target="main" class="text4">增加</a>
													                            		</td>
													                          		</tr>
													                          		<tr align="center" valign="middle">
													                          		...
													                          		<s:iterator id="firstwebarticle" value="pagination.list" status="userList">
													                          			<tr align="center" valign="middle">
																                            <td height="27">${userList.count}</td>
																                            <td height="27">${firstwebarticle.webTitle}</td>
																                            <td height="27">
																                            	<s:if test="#firstwebarticle.webContent.length()>10">
																                            		<a title="${firstwebarticle.webContent}"><s:property value="#firstwebarticle.webContent.substring(0,10)" /></a>
																                            	</s:if>
																                            	<s:else>
																                            		${firstwebarticle.webContent}
																                            	</s:else>
																                            	
																                            </td>
																                            <td height="27">${firstwebarticle.firstWebType.name}</td>
																                            
																                            <td height="27">
																	                           	<a href="<%=basePath%>/webart/webart_edit.action?id=${firstwebarticle.id}" style="cursor:pointer" class="text4">修改</a>&nbsp;&nbsp;  
																	                            <a class="deleteButton" name="${firstwebarticle.id}" style="cursor:pointer" class="text4"><s:text name="删除" /></a>
																                            </td>
														                           		</tr>
													                          		</s:iterator>
													                          			<tr>
